<template>
  <div class="rac-container">
    <section class="rac-menu" v-if="activeModuleText === '产业-资源-粮食'">
      ...
      <button class="rac-menu-item">
        ...
        <div class="rac-menu-item__sub-menu">
          ...
          <!--          </el-radio-group>-->
          <el-checkbox-group @change="guihuaChange" class="rac-menu-item__checkbox-group" v-model="state['产业-资源-粮食'].规划.activeMapLayer">
    ...
    <!--  ***************************************************  -->
    <section class="rac-menu" v-if="activeModuleText === '产业-资源-畜牧'">
      <button class="rac-menu-item">
        <img :src="icon3" class="rac-menu-item__icon" alt="icon" />
        规划
        <div class="rac-menu-item__sub-menu">
          <el-checkbox-group @change="guihuaChange" class="rac-menu-item__checkbox-group" v-model="state['产业-资源-畜牧'].规划.activeMapLayer">
    ...
    <!--  ***************************************************  -->
    <section class="rac-menu" v-if="activeModuleText === '产业-资源-水产' || (props.activeLayerCategoryTab == '专题图层' && selectValueList.includes('产业-资源-水产'))">
      <button class="rac-menu-item">
        <img :src="icon3" class="rac-menu-item__icon" alt="icon" />
        ...
        <div class="rac-menu-item__sub-menu">
          <button class="rac-menu-item">
            水产总布局
            <div class="rac-menu-item__sub-menu">
              <el-checkbox-group @change="guihuaChange" class="rac-menu-item__checkbox-group" v-model="state['产业-资源-水产'].规划.水产总布局.activeMapLayer">
          ...
          <button class="rac-menu-item">
            特色水产品养殖区域布局
            <div class="rac-menu-item__sub-menu">
              <el-checkbox-group @change="guihuaChange" class="rac-menu-item__checkbox-group" v-model="state['产业-资源-水产'].规划.特色水产品养殖区域布局.activeMapLayer">
    ...
    <!--  ***************************************************  -->
    <section class="rac-menu" v-if="activeModuleText === '产业-资源-蔬菜' || (props.activeLayerCategoryTab == '专题图层' && selectValueList.includes('产业-资源-蔬菜'))">
      <button class="rac-menu-item">
        <img :src="icon3" class="rac-menu-item__icon" alt="icon" />
        ...
        <div class="rac-menu-item__sub-menu">
          <button class="rac-menu-item">
            蔬菜总布局
            <div class="rac-menu-item__sub-menu">
              <el-checkbox-group @change="guihuaChange" class="rac-menu-item__checkbox-group" v-model="state['产业-资源-蔬菜'].规划.蔬菜总布局.activeMapLayer">
          ...
          <button class="rac-menu-item">
            特色品种区域布局
            <div class="rac-menu-item__sub-menu">
              <el-checkbox-group @change="guihuaChange" class="rac-menu-item__checkbox-group" v-model="state['产业-资源-蔬菜'].规划.特色品种区域布局.activeMapLayer">
    ...
    <!--  ***************************************************  -->
    <section class="rac-menu" v-if="activeModuleText === '产业-资源-水蜜桃'">
      <button class="rac-menu-item">
        <img :src="icon2" class="rac-menu-item__icon" alt="icon" />
        图例
        <div class="rac-menu-item__sub-menu">
    ...
    <!--  ***************************************************  -->
    <section class="rac-menu" v-if="activeModuleText === '产业-资源-食用植物油'">
      <button class="rac-menu-item">
        <img :src="icon3" class="rac-menu-item__icon" alt="icon" />
        规划
        <div class="rac-menu-item__sub-menu">
          <el-checkbox-group @change="guihuaChange" class="rac-menu-item__checkbox-group" v-model="state['产业-资源-食用植物油'].规划.activeMapLayer">
            <div class="rac-menu-item__checkbox-item">
    ...
    <!--  ***************************************************  -->
    <section class="rac-menu" v-if="activeModuleText === '产业-资源-粮功区内高标农'">
      <button class="rac-menu-item">
        <img :src="icon1" class="rac-menu-item__icon" alt="icon" />
        分析
        <div class="rac-menu-item__sub-menu">
          <button
            ...
            @click="
              () => {
                state['产业-资源-粮功区内高标农'].区域分布.visible = true
                _temp_onOpenModal('产业-资源-粮功区内高标农::区域分布')
              }
            "
          >
            区域分布
          ...
          <button
            ...
            @click="
              () => {
                state['产业-资源-粮功区内高标农'].三调分析.visible = true
                _temp_onOpenModal('产业-资源-粮功区内高标农::三调分析')
              }
            "
          >
            三调分析
          ...
          <button
            ...
            @click="
              () => {
                state['产业-资源-粮功区内高标农'].整治进展.visible = true
                _temp_onOpenModal('产业-资源-粮功区内高标农::整治进展')
              }
            "
          >
            整治进展
    ...
    <!--  ***************************************************  -->
    <!--  ***************************************************  -->
    <button class="rac-menu-item" v-if="Object.keys(availableCharts).includes(activeModuleText) && props.activeLayerCategoryTab != '专题图层'" @click="showChart = !showChart">
      目标<img :src="orangeArrow" class="rac-menu-item__arrow" alt="arrow" />
    </button>
    ...
    <custom-dialog v-model:visible="visibleMap['产业-资源-粮功区内高标农::三调分析']" :width="840" top="5vh" title="三调分析">
    ...
    <custom-dialog v-model:visible="visibleMap['产业-资源-粮功区内高标农::区域分布']" :width="840" top="5vh" title="区域分布">
    ...
    <custom-dialog v-model:visible="visibleMap['产业-资源-粮功区内高标农::整治进展']" :width="775" top="5vh" title="整治进展">
...
<script setup>
import { defineEmits, inject, defineProps, watch, reactive, ref, computed, onMounted, onBeforeUnmount, defineExpose, watchEffect } from 'vue'
...
const options = {
  ...
  水产总布局: [
    { name: '沿岸海洋渔业产业片区', color: '#9FAA45' },
    { name: '淡水渔业产业片区', color: '#73BA74' }
  ],
  特色水产品养殖区域布局: [
    { name: '海淡水虾类', color: '#FFAC1D' },
    { name: '海水贝藻类', color: '#CC8A1F' },
    { name: '海水蟹类', color: '#FD7E3F' },
    { name: '生态鳖', color: '#DBBB0D' },
    { name: '岱衢族大黄鱼', color: '#B29A20' }
  ],
  蔬菜总布局: [{ name: '蔬菜总布局', color: '#C85C9D' }],
  特色品种区域布局: [
    { name: '加工出口蔬菜（榨菜、花椰菜）', color: '#AD6A63' },
    { name: '加工出口蔬菜（雪菜）', color: '#FD7D7D' },
    { name: '鲜食大豆（蚕豌豆）', color: '#FF5C5C' },
    { name: '马铃薯', color: '#FC4646' },
    { name: '山地特色蔬菜（芋艿）', color: '#CC3333' },
    { name: '水生蔬菜（茭白）', color: '#C46796' },
    { name: '西甜瓜', color: '#FD76BA' },
    { name: '食用菌', color: '#FC33A9' },
    { name: '草莓', color: '#CC1176' }
  ],
  油菜籽重点乡镇: ['油菜籽重点乡镇'],
  油茶重点乡镇: ['油茶重点乡镇']
}

const otherParams = ref({ 产业基地规划面: [] })
const activeModuleText = ref('')
watch(
  () => {
    return JSON.stringify(props.changeLayerByOptionMessageObj)
  },
  () => {
    getActiveModuleText()
    clearCheckboxValue(activeModuleText.value)
  }
)
const getActiveModuleText = () => {
  let str = ['产业', '资源'].indexOf(props.activeModule) > -1 ? '产业-资源-' : ''
  let changeText = ''
  let text = ''
  if (str) {
    if (props.activeModule === '产业') {
      changeText = [].concat(props.changeLayerByOptionMessageObj.产业基地面, props.changeLayerByOptionMessageObj.产业基地点).filter((one) => one)[0]
    } else if (props.activeModule === '资源') {
      changeText = [].concat([props.changeLayerByOptionMessageObj.高标农内粮功区 || props.changeLayerByOptionMessageObj.高标准农田 ? '粮功区内高标农' : '']).filter((one) => one)[0]
    }
    text = str + changeText
  }
  activeModuleText.value = text
}
const clearCheckboxValue = (noClearKey) => {
  otherParams.value.产业基地规划面 = []
  for (let key in state) {
    if (key !== noClearKey && state[key].规划) {
      state[key].规划.activeMapLayer = []
    } else if (key === noClearKey && state[key].规划) {
      otherParams.value = { 产业基地规划面: state[key].规划.activeMapLayer || [] }
    }
  }
  sendMessage()
}
const guihuaChange = (v) => {
  otherParams.value.产业基地规划面 = v
  sendMessage()
}
const sendMessage = () => {
  emits('sendMessage', {
    ...
    value: {
      option: {
        name: '土地非农化',
        obj: cloneDeep(props.changeLayerByOptionMessageObj)
      }
    }
  })
}
...
onMounted(() => {
  emitter.on('themeLayerTypesChange', themeLayerTypesChange)
})
// 专题图层
const themeLayerTypesChange = (themeLayerValue) => {
  selectValueList.value = []
  let list = themeLayerValue.find((one) => one.name === '生产力布局').children.list.filter((one) => one.children)
  list.forEach((one) => {
    if (one.children && one.children.value && one.children.value.length) {
      selectValueList.value.push('产业-资源-' + one.name)
    }
  })
}
// start-三调分析 区域分布 整治进展
const selectValueList = ref([])
const visibleMap = ref({
  '产业-资源-粮功区内高标农::三调分析': false,
  '产业-资源-粮功区内高标农::区域分布': false,
  '产业-资源-粮功区内高标农::整治进展': false
})
const landType = ref('耕地')
const areaName = ref('')
const selectVal = computed(() => {
  let selectValText = inject('regionNamePath').value
  getCityArea(selectValText)
  isShowThreeCharts(selectValText)
  return selectValText
})
...
const getCityArea = (val) => {
  ...
  } else if (areaArr.length === (props.selectAreaVal.value ? 1 : 2)) {
    qyfbData.map((item) => {
      if (item.city === (props.selectAreaVal.value ? areaArr[0] : areaArr[1])) {
        cityArea.name.push(item.name)
        cityArea.lgq.push((+item.lgqmj).toFixed(1))
      }
    })
  } else if (areaArr.length === (props.selectAreaVal.value ? 2 : 3) || areaArr.length === (props.selectAreaVal.value ? 3 : 4)) {
    qyfbData.map((item) => {
      if (item.city === (props.selectAreaVal.value ? areaArr[1] : areaArr[2])) {
        cityArea.name.push(item.name)
        cityArea.lgq.push((+item.lgqmj).toFixed(1))
      }
    })
...
const availableModuleCharts = computed(() => {
  return availableCharts[activeModuleText.value]
})

watchEffect(() => {
  activeChart.value = availableCharts[activeModuleText.value]?.[0]?.name
})
watch([activeChart, currentRegionName], ([chart, regionName]) => {
  const chartConfig = availableCharts[activeModuleText.value]?.find((e) => e.name === chart)
  chartInstanceRef?.clear()
  if (chartConfig) {
    import(`@/assets/js/产业大脑/charts/${chartConfig.fileName}.json`)
      .then((v) => {
...
onBeforeUnmount(() => {
  ...
  emitter.off('themeLayerTypesChange', themeLayerTypesChange)
})
defineExpose({
  otherParams
})
